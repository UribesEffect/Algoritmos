...
void generate_vector(int size, vector<int>& v, minstd_rand0& rng){//aliatorios positivos
  v.clear();
...
void generate_vector2(int size, vector<int>& v, minstd_rand0& rng){//aleatorios numeros positivos y negativos
  v.clear();
  const int max_num = size;
  //Generation of a sequence of (pseudo)random numbers
  rng();
  for(int i = 0; i < size; ++i){
    int num = int(2*max_num * ( double(rng()) / rng.max() )-max_num);
    v.push_back(num);
  }
}
